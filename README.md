...
`GET /api/v1/payments/{payment_id}` или ждать вебхук. Тело вебхука:
...
Сумму лучше передавать строкой. С `example.com` в `webhook_url` вебхук, понятно, не
доставится и сообщение окажется в `payments.dlq` — чтобы увидеть доставку, подставьте
свой URL (например, с webhook.site).
...
- Consumer читает платёж из базы, вызывает шлюз (2-5 с, 90% успех), условным `UPDATE`
  ставит статус и шлет вебхук — до 3 попыток с паузами 1 и 2 с между ними. `4xx`
  (кроме 429) не ретраится.
